...
class PlotyHub:
    ...
    def adjust_single(cls, fig: Figure, ax: Axes, leg: Optional[Legend] = None, cbar: Optional[Colorbar] = None) -> Figure:
        ...
        fig_params = {
            ...
            # 边缘距离
            'left': 5. * cls.UNIT.cm, 'right': 1.5 * cls.UNIT.cm,
        ...
        # 坐标轴 参数
        ax_params = {
            'title_size': 20, 'title_x': 0.5, 'title_y': -0.25,
            'label_size': 22, 'label_pad': 7.,
        ...
        # 图例参数
        leg_params = {
            'font_size': 22, 'pad': 1, 'rounding_size': 0.8,
            }
        # 图例
        if leg is not None:
    ...
    def adjust_dual(
        ...
        fig_params = {
            ...
            # 子图间距
            'w_space': 16 * cls.UNIT.cm, 'h_space': 6.3 * cls.UNIT.cm,
            # 边缘距离
            'left': 6.5 * cls.UNIT.cm, 'right': 1.5 * cls.UNIT.cm,
            'top': 2.8 * cls.UNIT.cm, 'bottom': 5.5 * cls.UNIT.cm,
            }
        ...
        # 图例参数
        leg_params = {
            'font_size': 30, 'pad': 1.8, 'rounding_size': 0.8,
            }
        # 图例调整
        if leg1 is not None:
...
class PostProcess:
    ...
    def __init__(
        self,
        manager: opsu.pre.ModelManager, name: str,
        ...
        self.strain_stages = manager.get_param(category='section', label=name, key='strain_stages') # 应变阶段
        
        # 应变状态实例
        self.SMS = opsu.post.SecMatStates(odb_tag=name, ele_tag=self.ele_tag, integ=1, print_info=print_info)
        # 损伤状态
    ...
    def plot_equivalent_bilinear(
        self,
        line_x: Union[list, np.ndarray], line_y: Union[list, np.ndarray],
        point_idx: int, info: bool = True
        ) -> None:
        ...
        eq_x, eq_y = opsu.post.equivalent_bilinear(
            line_x=original_x, line_y=original_y,
            point_idx=point_idx, info=info
            )
        ...
        fig.suptitle(
            ...
            fontsize=22, color='black', x=0.5, y=0.98, alpha=1.0
            )
        # 坐标轴
        ax = plt.subplot2grid((1, 1), (0, 0), rowspan=1, colspan=1, facecolor='none')
        ...
        # 曲线数据
        ax.plot(
            original_x, original_y, label='Original',
            color="#03A9F4", linewidth=2.5,
            zorder=10
            )
        # 等效双线性数据
        ax.plot(
            [0., eq_x, x_end], [0, eq_y, eq_y], label='Equivalent',
            color="#E91E63", linewidth=2.5, marker='o', markersize=8,
            zorder=15
            )
        ax.scatter(
            x_key, y_key, label='KeyPoint',
            color="#FF9800", s=60,
            zorder=20
            )
        ...
        leg = ax.legend(
            loc='lower right',
            bbox_to_anchor=(0.97, 0.05), labelcolor=(0, 0, 0, 1),
        ...
        # 添加箭头
        ax.annotate(
            text='Equivalent yield', color='k',
            xytext=(eq_x * 2., eq_y * 0.6), textcoords='data',
            xy=(eq_x, eq_y), xycoords='data',
            fontsize=20,
            arrowprops=arrowprops,
            zorder=25
            )
        ax.annotate(
            text='Initial yield', color='k',
            xytext=(x_key * 2., y_key * 0.5), textcoords='data',
            xy=(x_key, y_key), xycoords='data',
            fontsize=20,
            arrowprops=arrowprops,
            zorder=25
            )
    ...
    def _sec_resp(self, SEC: opst.pre.section.FiberSecMesh, step: int) -> Figure:
        ...
        ax_sec, cbar_sec = self.SMS.plot_sec(
            SEC=SEC, data_type='Strains', step=step,
