...
class SectionHub:
    ...
    def sec_I(
        ...
        "# ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- -----"
        # 定义编号
        steel_tag = manager.next_tag(
            category="uniaxialMaterial", label=f"{sec_name}_steel"
        )  # 标签标记为绘图所需
        ...
        I_geom = geom_half + geom_half.mirror_section(
            axis="y", mirror_point=(0, 0)
        )  # 镜像叠加
        ...
        # ops 材料参数
        fy, Es = 400.0 * UNIT.mpa, 200.0 * UNIT.gpa
        ...
        # 定义截面
        SEC.to_opspy_cmds(secTag=sec_tag, G=100.0 * UNIT.gpa)
        ...
        # 储存至管理器
        manager.set_params(
            category="uniaxialMaterial", tag=steel_tag, params=mat_params
        )  # 材料
    ...
    def sec_rect(
        ...
        steel_type = "HRB500"
        fy, Es = (
            ReBarHub.get_fyk(steel_type) * UNIT.mpa,
            ReBarHub.get_Es(steel_type) * UNIT.mpa,
        )
        ...
        # 储存至管理器
        manager.set_params(
            category="uniaxialMaterial", tag=inner_tag, params=steel_params
        )  # 材料
        manager.set_params(category="section", tag=sec_tag, params=sec_props)  # 截面
    ...
    def sec_polygonal(
        ...
        "# ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- -----"
        # 形状参数
        bar_dia: float = 22.0 * UNIT.mm
        cover: float = 3.0 * UNIT.cm
        H1: float = 600.0 * UNIT.mm
        H2: float = 750.0 * UNIT.mm
        H3: float = 1350.0 * UNIT.mm
        W1: float = 500.0 * UNIT.mm
        W2: float = 1100.0 * UNIT.mm
        ...
        SEC.add_rebar_line(
            points=[
                (-(W1 / 2.0 - cover - bar_dia / 2.0), cover + bar_dia / 2.0),
                (W1 / 2.0 - cover - bar_dia / 2.0, cover + bar_dia / 2.0),
            ],
            dia=bar_dia,
            n=5,
        ...
        SEC.add_rebar_line(
            points=[
                (-(W1 / 2.0 - cover - bar_dia / 2.0), cover + bar_dia * 2.5),
                (W1 / 2.0 - cover - bar_dia / 2.0, cover + bar_dia * 2.5),
            ],
            dia=bar_dia,
            n=5,
        ...
        SEC.add_rebar_line(
            points=[
                (-(W2 / 2.0 - cover - bar_dia / 2.0), H3 - cover - bar_dia / 2.0),
                (W2 / 2.0 - cover - bar_dia / 2.0, H3 - cover - bar_dia / 2.0),
            ],
            dia=bar_dia,
            n=8,
        ...
        SEC.add_rebar_line(
            points=[
                (-(W2 / 2.0 - cover - bar_dia / 2.0), H3 - cover - bar_dia * 2.5),
                (W2 / 2.0 - cover - bar_dia / 2.0, H3 - cover - bar_dia * 2.5),
            ],
            dia=bar_dia,
            n=8,
        ...
        SEC.add_rebar_line(
            points=[
                (W2 / 2.0 - cover - bar_dia / 2.0, H3 - bar_dia * 10.0),
                (W2 / 2.0 - cover - bar_dia / 2.0, H2 + bar_dia),
                (W1 / 2.0 - cover - bar_dia / 2.0, H1 + bar_dia),
                (W1 / 2.0 - cover - bar_dia / 2.0, 0.0 + bar_dia * 10.0),
            ],
            dia=bar_dia,
            n=8,
            ops_mat_tag=rebar_tag,
        ...
        # 素混凝土
        fc, Ec = (
            ConcHub.get_fcuk(concrete_type) * UNIT.mpa,
            ConcHub.get_Ec(concrete_type) * UNIT.mpa,
        )
        ec, ecu = 0.002, 0.0033
        # 核心混凝土
        ...
        # 钢筋
        fy, Es = (
            ReBarHub.get_fyk(rebar_type) * UNIT.mpa,
            ReBarHub.get_Es(rebar_type) * UNIT.mpa,
        )
        ...
        # 定义截面
        SEC.to_opspy_cmds(
            secTag=sec_tag, GJ=sec_props["J"] * ConcHub.get_G(concrete_type) * UNIT.mpa
        )
    ...
    def sec_circle(
        ...
        R: float = 80.0 * UNIT.cm
        r: float = 20.0 * UNIT.cm

        # 轮廓线
        cover_outline = opst.pre.section.create_circle_points(
            xo=[0, 0], radius=R - cover, n_sub=30
        )  # 保护层轮廓线
        inner_outline = opst.pre.section.create_circle_points(
            xo=[0, 0], radius=r, n_sub=30
        )  # 内孔轮廓线
        # 生成网格形状
        cover_geo = opst.pre.section.create_circle_patch(
            xo=[0, 0], radius=R, holes=[cover_outline], n_sub=30
        )  # 保护层几何形状
        core_geo = opst.pre.section.create_circle_patch(
            xo=[0, 0], radius=R - cover, holes=[inner_outline], n_sub=30
        )  # 核心几何形状
