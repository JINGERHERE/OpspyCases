...
"""
# ===== ===== ===== ===== ===== ===== ===== ===== ===== =====
@File    ：Part_MatSec_DoubleColHeighPier.py
@Date    ：2025/08/10 11:55:41
...
"""
# --------------------------------------------------
# ========== < Part_MatSec_DoubleColHeighPier > ==========
# --------------------------------------------------
"""


class HighPierModelSection:
    ...
    def pier_sec(
        ...
        "# ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- -----"
        # 材料控制
        fc = 35.4 * UNIT.mpa  # 混凝土抗压强度
        Ec = 31.4 * UNIT.gpa  # 混凝土弹性模量

        fy = 364 * UNIT.mpa  # 钢筋屈服强度
        Es = 200 * UNIT.gpa  # 钢筋弹性模量

        # 截面控制
        W = 5200 * UNIT.mm # 5200
        H = 8500 * UNIT.mm # 8500
        t = 1500 * UNIT.mm # 截面壁厚
        cover = 100 * UNIT.mm  # 保护层厚度
        dia_bar = 50 * UNIT.mm  # 钢筋直径
        n_bar_out = 170  # 外圈纵筋个数
        n_bar_in = 110  # 内圈纵筋个数
        ...
        SEC.set_mesh_size({"cover": 1 * UNIT.m, "core": 1 * UNIT.m})
        ...
        Acor = (W - 2 * (dia_bar + cover)) * (H - 2 * (dia_bar + cover)) * (100 * UNIT.mm)  # 核心面积 x 1米
        As = pi * (10 / 2 * UNIT.mm) ** 2  # 箍筋单肢面积
        RectangularSection.set_params(
            ...
            dsl=dia_bar,  # 纵筋直径
            # roux = (7 * As * (W - 2 * (cover + dia_bar))) / Acor, # x方向的体积配箍率, 计算时只计入约束混凝土面积
            # rouy = (7 * As * (H - 2 * (cover + dia_bar))) / Acor, # y方向的体积配箍率, 计算时只计入约束混凝土面积
            roux = 1.2 / 100,
            rouy = 1.2 / 100,
            st=1000 * UNIT.mm,  # 箍筋间距
            dst=10 * UNIT.mm,  # 箍筋直径
            fyh=338 * UNIT.mpa,  # 箍筋屈服强度(MPa)
